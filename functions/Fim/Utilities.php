...
class Utilities {
     ...
     */
    public static function objectArrayFilterKeys($object, array $keys) : array {
        $newArray = [];
     ...
     */
    public static function naturalLanguageJoin(string $glue, array $list, string $conjunction = 'and') {
        $last = array_pop($list);
        if ($list) {
     ...
     */
    public static function arrayOfPropertiesImplode(&$array) {
        sort($array);
    ...
    public static function dbCastArrayEntry(array &$array, $keys, $cast) {
    ...
    public static function castArrayEntry(array $array, $keys, $cast) {
        foreach ((array) $keys AS $key) {
            if (isset($array[$key])) {
     ...
     * @return array
     */
    public static function removeNullValues(array &$a) {
     ...
     * @return array
     */
    public static function sanitizeGPC($type, $data) {
        $newData = [];
                    ...
                    if (isset($indexMetaData['conflict'])) {
                        foreach ($indexMetaData['conflict'] AS $conflict) {
                            if (isset($activeGlobal[$conflict])) {
                                ...
                                new \Fim\Error(\Fim\Utilities::arrayOfPropertiesImplode($conflictArray) . 'Conflict');
                            }
                        }
                    }
                ...
                switch($indexMetaData['cast']) {
                    ...
                    case 'dict':
                        ...
                        if (!is_array($activeGlobal[$indexName]))
                            new \Fim\Error("{$indexName}NotArray", "API parameter '$indexName' must be an array.");
                        ...
                        $newData[$indexName] = \Fim\Utilities::arrayValidate(
                            $arrayFromGlobal,
                     ...
                     */
                    case 'roomId':
                        $newData[$indexName] = @self::cast('roomId', $activeGlobal[$indexName]);
     ...
     * @return mixed
     */
    public static function cast($cast, $value, $default = null) {
        switch ($cast) {
            case 'bool':
     ...
     * @return array
     */
    public static function arrayValidate($array, $type = 'int', $preserveAll = false, $allowedValues = false) {
        ...
        if (is_array($array)) { // Make sure the array is an array.
            foreach ($array AS $key => $value) { // Run through each value of the array.
                ...
                $preValue = @self::cast($type, $value, false);
     ...
     * @return bool
     */
    public static function inArray(array $needle, array $haystack, $all = false) {
        if (!$haystack)
            return false; // If the haystack is not valid, return false.

        elseif (!$needle)
            return false; // If the needle is not valid, return false.

        else {
            foreach($needle AS $need) { // Run through each entry of the needle
                if ($all) { // All values must be found.
                    if (!$need)
                        return false; // If the needle value is false, skip it.
                    if (in_array($need, $haystack))
                        continue; // If the needle is in the haystack, return true.
                }
                else { // Only one value must be found.
                    if (!$need)
                        continue; // If the needle value is false, skip it.
                    if (in_array($need, $haystack))
                        return true; // If the needle is in the haystack, return true.
                }
            }

            return (bool) $all; // Return true only if all values found.
        }
    }
     ...
     */
    public static function dobToAge($date) {
     ...
     * @return mixed
     */
    public static function nearestAge($age) {
    ...
    public static function buildSelect($selectName, $selectArray, $selectedItem) {
