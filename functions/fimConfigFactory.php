...
class fimConfigFactory
{
    ...
    public static function init(fimDatabase $database)
    {
        ...
        elseif (function_exists('apcu_exists') && apcu_exists('fim_config')) {
            ...
            if (!$disableConfig) {
                foreach ($database->getConfigurations()->getAsArray(true) AS $configDatabaseRow) {
                    switch ($configDatabaseRow['type']) {
                        ...
                        case 'bool':
                            // We include the non-string counterparts here on the off-chance the database driver supports returning non-strings. The third parameter in the in_array makes it a strict comparison.
                            if (in_array($configDatabaseRow['value'], ['true', '1', true, 1], true))
                                $config::${$configDatabaseRow['directive']} = true;
                            else
                                $config::${$configDatabaseRow['directive']} = false;
                            break;
