...
function fim_buildUsernameTag(a,b,c,d){$.when(c).then(function(c){var e=c[b].name,g=c[b].nameFormat,f=c[b].avatar?c[b].avatar:"client/images/blankperson.png";a.attr({"class":"userName"+(settings.showAvatars||d?" userNameAvatar":""),style:settings.showAvatars?"":g,"data-userId":b,"data-userName":e,"data-avatar":f,tabindex:1E3}).append(settings.showAvatars||d?$("<img>").attr({alt:e,src:f?f:"client/images/blankperson.png"}):"").append(!settings.showAvatars||d?$("<span>").text(e):"");a.popover({content:function(){var a=
...
month:"numeric",day:"numeric"}))));a.append($('<div class="col-sm-auto">').append($('<img style="max-height: 200px; max-width: 200px;" class="mr-2">').attr("src",f)),d);return a.prop("outerHTML")},html:!0,trigger:"hover"}).on("show.bs.popover",function(a){console.log($(this).data("bs.popover"),$(this).data("bs.popover").tip);$($(this).data("bs.popover").tip).css({"max-width":"600px"})})});return a}
...
$.when(h).then(function(a){g.attr("style",a[c].messageFormatting)});return g}function fim_getUsernameDeferred(a){return $.when(Resolver.resolveUsersFromIds([a]))}function fim_getRoomNameDeferred(a){return $.when(Resolver.resolveRoomsFromIds([a]))}
...
function updateVids(a){jQTubeUtil.search(a,function(a){var c="",b=0;for(vid in a.videos){var e=a.videos[vid];b+=1;1===b%3&&(c+="<tr>");c+='<td><img src="http://i2.ytimg.com/vi/'+e.videoId+'/default.jpg" style="width: 120px; height: 90px;" /><br /><small><a href="javascript: void(0);" onclick="youtubeSend(&apos;'+e.videoId+'&apos;)">'+e.title+"</a></small></td>";0===b%3&&(c+="</tr>")}0!==b%3&&(c+="</tr>");$("#youtubeResults").html(c);return!1});return!1}
var autoEntry=function(a,b){this.options=b;var c=this;a.append($("<form>").attr("class","input-group").append($("<span>").attr("class","input-group-btn").append($("<button>").attr({"class":"btn btn-success"}).text("Add")),$("<input>").attr({type:"text",name:this.options.name+"Bridge",id:this.options.name+"Bridge",class:"ui-autocomplete-input form-control",autocomplete:"off"}).autocompleteHelper(this.options.list)).submit(function(){c.addEntry($("#"+c.options.name+"Bridge").attr("data-id"),$("#"+c.options.name+
"Bridge").val());return!1}));$('<input type="hidden" name="'+this.options.name+'" id="'+this.options.name+'">').insertAfter(a);$("<div>").attr("id",this.options.name+"List").insertAfter(a);"default"in b&&this.displayEntries(b.default);return this};
...
"object"===typeof a?a:"string"===typeof a&&0<a.length?a.split(","):[];var c=this;$.when(this.options.resolveFromIds(b)).then(function(a){for(var d=0;d<b.length;d++)c.addEntry(b[d],a[b[d]].name,!0)})},getList:function(){return $("#"+this.options.name).val().split(",").filter(Number)}};function windowDraw(){console.log("Redrawing window.");windowResize();return!1}
