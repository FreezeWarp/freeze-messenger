...
function fim_createParentalControls(a){jQuery.each(window.serverSettings.parentalControls.parentalAges,function(b,c){$("select[name=parentalAge]",a).append($("<option>").attr("value",c).text($l("parentalAges."+c)))});jQuery.each(window.serverSettings.parentalControls.parentalFlags,function(b,c){$("[name=parentalFlagsList]",a).append($("<label>").attr("class","btn btn-secondary m-1").text($l("parentalFlags."+c)).prepend($("<input>").attr({type:"checkbox",value:c,name:"parentalFlags"})))})}
var popup=function(){};
...
$("#loginForm");standard.initialLogin({username:$("input[name=userName]",c).val(),password:$("input[name=password]",c).val(),finish:a,error:b});return!1});$("#modal-login").on("hidden.bs.modal",function(){window.userId||standard.initialLogin({username:"",password:"",rememberMe:!1,finish:a,error:b})})};
...
var c=serverSettings.fileUploads.fileContainers[b],d=serverSettings.fileUploads.extensionChangesReverse[b];$("table#fileUploadInfo tbody").append("<tr><td>"+(d?d.join(", "):b)+"</td><td>"+$l("fileContainers."+c)+"</td><td>"+$.formatFileSize(a,$l("byteUnits"))+"</td></tr>")}),"function"!==typeof FileReader?$("#uploadFileForm").html($l("uploadErrors.notSupported")):(window.serverSettings.parentalControls.parentalEnabled?fim_createParentalControls($("form#uploadFileForm")):$("#insertDocParentalAge, #insertDocParentalFlags").remove(),
$("#uploadFileForm").submit(function(){filesList=$('input#fileUpload[type="file"]').prop("files");0==filesList.length?dia.error("Please select a file to upload."):($("#fileUpload").fileupload(),$("#fileUpload").fileupload("add",{files:filesList,url:directory+"api/editFile.php?"+$.param({_action:"create",uploadMethod:"put",dataEncode:"binary",roomId:window.roomId,fileName:filesList.item(0).name,access_token:window.sessionHash,parentalAge:$("form#uploadFileForm select[name=parentalAge] option:selected").val(),
parentalFlags:$("form#uploadFileForm input[name=parentalFlags]:checked").map(function(){return $(this).attr("value")}).get()}),type:"PUT",multipart:!1}),$("#fileUpload").fileupload("destroy"),$("#modal-insertDoc").modal("hide"));return!1}),$("#fileUpload").bind("change",function(){var e=new FileReader,f=new FileReader;console.log("FileReader triggered.");$("#imageUploadSubmitButton").attr("disabled","disabled").button({disabled:!0});0===this.files.length?dia.error("No files selected!"):1<this.files.length?
dia.error("Too many files selected!"):(console.log("FileReader started."),a=this.files[0].name,b=this.files[0].size,c=a.split("."),d=c[c.length-1],d in serverSettings.fileUploads.extensionChanges&&(d=serverSettings.fileUploads.extensionChanges[d]),-1===$.inArray(d,$.toArray(serverSettings.fileUploads.allowedExtensions))?$("#uploadFileFormPreview").html($l("uploadErrors.badExtPersonal")):b>serverSettings.fileUploads.sizeLimits[d]?$("#uploadFileFormPreview").html($l("uploadErrors.tooLargePersonal",
{fileSize:serverSettings.fileUploads.sizeLimits[d]})):($("#uploadFileFormPreview").html("Loading Preview..."),e.readAsBinaryString(this.files[0]),e.onloadend=function(){window.btoa(e.result)},f.readAsDataURL(this.files[0]),f.onloadend=function(){$("#uploadFileFormPreview").html(fim_messagePreview(serverSettings.fileUploads.fileContainers[d],this.result))},$("#imageUploadSubmitButton").removeAttr("disabled").button({disabled:!1})))}))):$("#insertDocUpload").html("Disabled.");return!1};
...
a.id)}});window.serverSettings.parentalControls.parentalEnabled?(fim_createParentalControls($("#changeSettingsForm")),jQuery.each(a.parentalFlags,function(a,b){$("input[name=parentalFlags][value="+b+"]").attr("checked",!0)})):$("#settings5parentalAge, #settings5parentalFlags").hide();$('input[name=privacyLevel][value="'+a.privacyLevel+'"]').prop("checked",!0)}});"vanilla"!==window.serverSettings.branding.forumType&&$("#settings5profile").hide(0);$("#defaultRoom").autocompleteHelper("rooms");window.webproDisplay.theme&&
$('#theme > option[value="'+window.webproDisplay.theme+'"]').attr("selected","selected");$("#theme").change(function(){$("#stylesjQ").attr("href","client/css/"+this.value+"/jquery-ui-1.8.16.custom.css");$("#stylesVIM").attr("href","client/css/"+this.value+"/fim.css");$.cookie("webpro_theme",this.value,{expires:14});window.webproDisplay.theme=this.value;return!1});window.webproDisplay.fontSize&&$('#fontsize > option[value="'+window.webproDisplay.fontSize+'"]').attr("selected","selected");$("#fontsize").change(function(){$("body").css("font-size",
this.value+"em");$.cookie("webpro_fontsize",this.value,{expires:14});window.webproDisplay.fontSize=this.value;windowResize();return!1});snd.volume&&$("#audioVolume").attr("value",100*snd.volume);$("#audioVolume").change(function(){$.cookie("webpro_audioVolume",this.value,{expires:14});snd.volume=this.value/100;return!1});settings.showAvatars&&$("#showAvatars").attr("checked","checked");settings.reversePostOrder&&$("#reversePostOrder").attr("checked","checked");settings.disableFormatting&&$("#disableFormatting").attr("checked",
"checked");settings.disableVideo&&$("#disableVideo").attr("checked","checked");settings.disableImage&&$("#disableImage").attr("checked","checked");$("#showAvatars, #reversePostOrder, #disableFormatting, #disableVideo, #disableImage").change(function(){var b=$(this).attr("id");$(this).is(":checked")&&!settings[b]?(settings[b]=!0,$("#messageList").html(""),$.cookie("webpro_settings",Number($.cookie("webpro_settings"))+a[b],{expires:14})):!$(this).is(":checked")&&settings[b]&&(settings[b]=!1,$("#messageList").html(""),
$.cookie("webpro_settings",Number($.cookie("webpro_settings"))-a[b],{expires:14}));standard.changeRoom(window.roomId)});settings.audioDing&&$("#audioDing").attr("checked","checked");settings.disableFx&&$("#disableFx").attr("checked","checked");settings.disableRightClick&&$("#disableRightClick").attr("checked","checked");settings.webkitNotifications&&$("#webkitNotifications").attr("checked","checked");$("#audioDing, #disableFx, #webkitNotifications, #disableRightClick").change(function(){var b=$(this).attr("id");
$(this).is(":checked")&&!settings[b]?(settings[b]=!0,$.cookie("webpro_settings",Number($.cookie("webpro_settings"))+a[b],{expires:14}),"disableFx"===b&&(jQuery.fx.off=!0),"webkitNotifications"===b&&(notify.webkitNotifySupported()?notify.webkitNotifyRequest():dia.error("Notifications are not supported on your browser."))):!$(this).is(":checked")&&settings[b]&&(settings[b]=!1,$.cookie("webpro_settings",Number($.cookie("webpro_settings"))-a[b],{expires:14}),"disableFx"===b&&(jQuery.fx.off=!1))});$("#changeSettingsForm").submit(function(){var a=
...
serverSettings.parentalControls.parentalEnabled?fim_createParentalControls($("#editRoomForm")):$("#editRoom1ParentalAge, #editRoom1ParentalFlags").remove();fimApi.getCensorLists({roomId:roomId?roomId:null,includeWords:0},{each:function(a){if(a.status)var b=a.status;else if("white"===a.listType)b="block";else if("black"===a.listType)b="unblock";else throw"Bad logic.";$("#censorLists").append($("<label>").attr("class","btn btn-secondary m-1").text(a.listName).prepend($("<input>").attr({type:"checkbox",
name:"censorLists",value:a.listId}).attr(a.listOptions&2?{disabled:"disabled"}:{}).attr("block"==b?{checked:"checked"}:{})))}});roomId&&fimApi.getRooms({id:roomId},{each:function(a){var b=[],c=[];jQuery.each(a.userPermissions,function(a,d){d.moderate&&d.properties&&d.grant&&c.push(a);d.post&&b.push(a)});allowedUsersList.displayEntries(b);moderatorsList.displayEntries(c);var f=[];jQuery.each(a.groupPermissions,function(a,b){b.post&&f.push(a)});allowedGroupsList.displayEntries(f);"view"in a.defaultPermissions&&
$("#allowViewing").prop("checked",!0);"post"in a.defaultPermissions&&$("#allowPosting").prop("checked",!0);$("#name").val(a.name);$("#allowOfficial").prop("checked",a.official);$("#allowHidden").prop("checked",a.hidden);jQuery.each(a.parentalFlags,function(a,b){$("#editRoomForm input[name=parentalFlags][value="+b+"]").prop("checked",!0)});$("#editRoomForm select[name=parentalAge] option[value="+a.parentalAge+"]").attr("selected","selected")}});$("#editRoomForm").submit(function(){console.log("allowed users",
allowedUsersList,allowedUsersList.getList());var a=$("#name").val(),d={},e={},f={};"create"===b&&(allowedUsersList.getList().forEach(function(a){e["+"+a]=["view","post"]}),moderatorsList.getList().forEach(function(a){e["+"+a]=["view","post","moderate"]}),allowedGroupsList.getList().forEach(function(a){f["+"+a]=["post"]}));$("input[name=censorLists]").each(function(){d[$(this).attr("value")]=$(this).is(":checked")?1:0});defaultPermissions=[];$("#allowViewing").is(":checked")&&defaultPermissions.push("view");
$("#allowPosting").is(":checked")&&defaultPermissions.push("post");fimApi.editRoom(roomId,b,{name:a,defaultPermissions:defaultPermissions,userPermissions:e,groupPermissions:f,parentalAge:$("#editRoomForm select[name=parentalAge] option:selected").val(),parentalFlags:$("input[name=parentalFlags]:checked").map(function(){return $(this).attr("value")}).get(),censorLists:d,official:$("#allowOfficial").is(":checked"),hidden:$("#allowHidden").is(":checked")},{end:function(a){dia.full({content:'The room has been created at the following URL: <br /><br /><form action="'+
...
a.string?(window.roomId=!1,window.location.hash="#rooms",dia.error("That room doesn't exist. Please select a room.")):fimApi.getDefaultExceptionHandler()(a)}});fimApi.getActiveUsers({roomIds:[roomId]},{refresh:15E3,timerId:1,begin:function(){$("#activeUsers").html("<ul></ul>")},each:function(a){$("#activeUsers > ul").append($("<li>").append(fim_buildUsernameTag($("<span>"),a.id,fim_getUsernameDeferred(a.id),!0)))}})},setRoom:function(a){this.options.roomId!=a&&this.init({roomId:a})},eventListener:function(){var a=
this,b=new EventSource(directory+"stream.php?queryId="+a.options.roomId+"&streamType=room&lastEvent="+a.options.lastEvent+"&lastMessage="+a.options.lastMessage+"&access_token="+window.sessionHash),c=function(b){return function(d){d.id>a.options.lastEvent&&(a.options.lastEvent=d.id);b(JSON.parse(d.data))}};b.addEventListener("newMessage",c(function(b){a.options.lastMessage=Math.max(a.options.lastMessage,b.id);fim_newMessage(a.options.roomId,Number(b.id),fim_messageFormat(b,"list"))}),!1);b.addEventListener("topicChange",
c(function(a){$("#topic").html(a.param1);console.log("Event (Topic Change): "+a.param1)}),!1);b.addEventListener("deletedMessage",c(function(a){$("#message"+a.id).fadeOut()}),!1);b.addEventListener("editedMessage",c(function(b){0<$("#message"+b.id).length&&(b.userId=$("#message"+b.id+" .userName").attr("data-userid"),b.time=$("#message"+b.id+" .messageText").attr("data-time"),fim_newMessage(a.options.roomId,Number(b.id),fim_messageFormat(b,"list")))}),!1)},getMessages:function(){var a=this;a.options.roomId?
fimApi.getMessages({roomId:a.options.roomId},{autoId:!0,refresh:3E3,each:function(b){fim_newMessage(Number(a.options.roomId),Number(b.id),fim_messageFormat(b,"list"))},end:function(){window.requestSettings.serverSentEvents&&(fimApi.getMessages(null,{close:!0}),a.eventListener())}}):console.log("Not requesting messages; room undefined.");return!1},populateMessages:function(){var a=this;$(document).ready(function(){$("#messageList").html("");window.requestSettings[a.options.roomId]={lastMessage:null,
firstRequest:!0};messageIndex[a.options.roomId]=[];a.getMessages();a.options.intervalPing&&clearInterval(a.options.intervalPing);fimApi.ping(a.options.roomId);a.options.intervalPing=window.setInterval(function(){fimApi.ping(a.options.roomId)},3E5);windowDraw();windowDynaLinks()})},sendMessage:function(a,b,c){var d=this;d.options.roomId?fimApi.sendMessage(d.options.roomId,{ignoreBlock:1===b?1:"",message:a,flag:c?c:""},{end:function(a){"censor"in a&&0<Object.keys(a.censor).length&&dia.info(Object.values(a.censor).join("<br /><br />"),
"Censor warning: "+Object.keys(a.censor).join(", "))},exception:function(b){"confirmCensor"===b.string?dia.confirm({text:b.details,"true":function(){d.sendMessage(a,1,c)}},"Censor Warning"):"spaceMessage"===b.string?dia.error("Too... many... spaces!"):fimApi.getDefaultExceptionHandler()(b)},error:function(b){settings.reversePostOrder?$("#messageList").append('Your message, "'+a+'", could not be sent and will be retried.'):$("#messageList").prepend('Your message, "'+a+'", could not be sent and will be retried.');
window.setTimeout(function(){d.sendMessage(a)},5E3);return!1}}):window.location.hash="#rooms"},disableSender:function(){$("#messageInput").attr("disabled","disabled");$("#icon_url").button({disabled:!0});$("#icon_submit").button({disabled:!0});$("#icon_reset").button({disabled:!0})},enableSender:function(){$("#messageInput").removeAttr("disabled");$("#icon_url").button({disabled:!1});$("#icon_submit").button({disabled:!1});$("#icon_reset").button({disabled:!1})}};
...
fimApi.getRooms({page:this.options.page},{each:function(a){$("#roomTableHtml").append($("<tr>").attr("id","room"+a.id).append($("<td>").append($("<a>").attr("href","#room="+a.id).text(a.name)),$("<td>").text(a.topic),$("<td>").append($("<div>").append(a.permissions.properties?$("<button>").attr({"class":"btn btn-secondary m-1",title:"Edit Room"}).text("Edit").prepend($('<i class="fa fa-sliders"></i>')).click(function(){window.location.hash="#editRoom#room="+a.id}):"",a.permissions.properties?$("<button>").attr({"class":"btn btn-danger m-1",
title:"Delete Room"}).text("Delete").prepend($('<i class="fa fa-trash"></i>')).click(function(){dia.confirm("Are you sure you want to delete this room?")&&standard.deleteRoom(a.id)}):"",$("<button>").attr({"class":"btn btn-secondary m-1",title:"View History"}).text("Archive").prepend($('<i class="fa fa-history"></i>')).click(function(){window.location.hash="#archive#room="+a.id}),$("<button>").attr({"class":"btn btn-secondary m-1",title:"Favourite Room"}).text("Fav").prepend($('<i class="fa fa-star" style="color: yellow;"></i>')),
$("<button>").attr({"data-toggle":"button","class":"btn btn-secondary m-1"+(-1!==window.activeLogin.userData.watchRooms.indexOf(a.id)?" active":""),"aria-pressed":-1!==window.activeLogin.userData.watchRooms.indexOf(a.id)?"true":"false",title:"Get Notifications About New Messages in This Room"}).text("Watch").prepend($('<i class="fa fa-eye"></i>')).click(function(){-1===window.activeLogin.userData.watchRooms.indexOf(a.id)?(dia.info("You will now be notified of new messages made in this room.","Now Watching"),
window.activeLogin.userData.watchRooms.push(a.id),fimApi.watchRoom(a.id),$(this).addClass("active")):(window.activeLogin.userData.watchRooms.remove(a.id),fimApi.unwatchRoom(a.id),$(this).removeClass("active"))})))))}})},nextPage:function(){$("#archivePrev").button({disabled:!1});this.options.page++;this.retrieve()},prevPage:function(){0!==this.options.page&&this.options.page--;0>=this.options.page&&$("#archivePrev").button({disabled:!0});this.retrieve()},update:function(a,b){this.options[a]=b}};
