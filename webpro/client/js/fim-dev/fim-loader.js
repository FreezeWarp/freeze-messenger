...
if (typeof Date === 'undefined') {
  ...
  throw new Error(window.phrases.errorBrowserDate);
}
else if (typeof Math === 'undefined') {
  window.location.href = 'browser.php';
  throw new Error(window.phrases.errorBrowserMath);
}
else if (false === ('encodeURIComponent' in window || 'escape' in window)) {
  window.location.href = 'browser.php';
  throw new Error(window.phrases.errorBrowserEscape);
}
...
function fim_dateFormat(timestamp, full) {
  ...
  _timepart = {
    seconds: function () {
      return _zeropad(jsdate.getSeconds(), 2);
    }, // Seconds
    minutes: function () {
      return _zeropad(jsdate.getMinutes(), 2);
    }, // Minutes
    hours: function () {
      return _zeropad((jsdate.getHours() % 12 || 12), 2);
    }, // 12-Hours
    hours24: function () {
      return _zeropad(jsdate.getHours(), 2);
    }, // 24-Hours
    days: function () {
      return _zeropad(jsdate.getDate(), 2);
    }, // Days
    months: function () {
      return _zeropad(jsdate.getMonth() + 1, 2);
    }, // Month
    years: function () {
  ...
  if (!full) {
    ...
    if (timestamp < lastMidnight) {
      full = true;
    } // If the current time is before the last midnight...
  }
...
function fim_messagePreview(container, content) {
  switch (container) {
    case 'image':
      return '<img src="' + content + '" style="max-height: 200px; max-width: 200px;" />';
      break;
    case 'video':
      return '<video src="' + content + '" style="max-height: 200px; max-width: 200px;">Video Preview Not Supported</video>';
      break;
    case 'audio':
      return '<audio src="' + content + '" style="max-height: 200px; max-width: 200px;">Audio Preview Not Supported</video>';
      break;
    case 'text':
        ...
        success: function (json) {
          ...
          for (i in json.getUsers.users) {
            ...
            if ('parentalFlags' in json.getUsers.users[i]) { // Normally, only for the logged in user.
              json.getUsers.users[i].parentalFlags = $.map(json.getUsers.users[i].parentalFlags, function (value, key) {
                return value;
              }); // The map function here will convert the object to an array.
            }
...
autoEntry = {
  ...
  showEntries: function (type, string) {
    ...
    if (typeof string === 'object' || typeof string === 'array') {
      entryList = string;
    } // String is already not a string! (yeah...) Also, "array" doesn't exist as a type far as I know, but I don't really want to remove it for whatever reason.
    else if (typeof string === 'string' && string.length > 0) {
      entryList = string.split(',');
    } // String is a string and not empty.
    else {
      entryList = [];
    }
...
function windowDraw() {
  ...
  if (roomId && (userId | anonId)) { /* TODO */
  } // The user is able to post.
  else {
    disableSender();
  } // The user is _not_ able to post.
...
function windowDynaLinks() {
  ...
  if (roomId) {
    if (roomId.toString().substr(0, 1) === 'p' || modRooms[roomId] < 1) {
      $('li > #kick').parent().hide();
    ...
    if (roomId.toString().substr(0, 1) === 'p' || modRooms[roomId] < 2) {
...
$(document).ready(function () {
  ...
  $('#roomList, #roomList2').bind('click', function () {
    popup.selectRoom();
  }); // Room List
