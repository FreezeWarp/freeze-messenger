...
window.userId&&window.activeLogin.userData.permissions.editOwnPosts}},click:{name:"URL",callback:function(){dia.full({title:"Copy Image URL",content:$("<div>").append($("<img>").attr({src:$(this).attr("src"),style:"max-width: 550px; max-height: 550px; margin-left: auto; margin-right: auto; display: block;"}),$("<br>"),$("<br>"),$("<input>").attr({type:"text",name:"url",value:$(this).attr("src"),style:"width: 100%"})).prop("outerHTML"),width:800,position:"top",oF:function(){$("input[name=url]",this).first().focus()}})}}}});
