...
function fim_youtubeParse(a){if(a.match(regexs.youtubeFull)||a.match(regexs.youtubeShort)){var b="";null!==a.match(regexs.youtubeFull)?b=a.replace(regexs.youtubeFull,"$8"):null!==a.match(regexs.youtubeShort)&&(b=a.replace(regexs.youtubeShort,"$5"));return settings.disableVideo?$("<a>").attr({href:"https://www.youtu.be/"+b,target:"_BLANK"}).text("[Youtube Video]"):$('<div class="video-container">').append($("<iframe>").attr({width:560,height:315,src:"https://www.youtube.com/embed/"+b+"?rel=0",frameborder:0,
allowfullscreen:!0}))}return fim_formatAsUrl(a)}function fim_formatAsImage(a){return $('<a target="_BLANK" class="imglink">').attr("href",a).append(settings.disableImage?$("<span>").text("[IMAGE]"):$('<img class="inlineImage" />').attr("src",a+(a.slice(0,window.serverSettings.installUrl.length)===window.serverSettings.installUrl?"&"+$.param({thumbnailWidth:400,thumbnailHeight:400}):"")))}
...
/^\/me/.test(c.text())?(c.text(c.text().replace(/^\/me/,"")),$.when(l).then(function(a){c.html($('<span style="color: red; padding: 10px; font-weight: bold;">').text("* "+a[g].name+" "+c.text()).prop("outerHTML"))})):/^\/topic/.test(c.text())&&(c.text(c.text().replace(/^\/topic/,"")),$.when(l).then(function(a){c.html($('<span style="color: red; padding: 10px; font-weight: bold;">').text("* "+a[g].name+' changed the topic to "'+c.text().trim()+'".').prop("outerHTML"))})),jQuery.each({"```":"pre","`":"code"},
function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(a){return $("<"+b+">").text(a[1])}).html())})}),c.find("*").add(c).contents().filter(function(){return 3===this.nodeType}).each(function(){var a=fim_regexTokenizer(regexs.url,$(this).text(),function(a){var b="";a[0].match(regexs.url2)&&(b=a[0].replace(regexs.url2,"$2"),a[0]=a[0].replace(regexs.url2,"$1"));
return a[0].match(regexs.youtubeFull)||a[0].match(regexs.youtubeShort)?fim_youtubeParse(a[0]).append(document.createTextNode(b)):a[0].match(regexs.image)?fim_formatAsImage(a[0]).append(document.createTextNode(b)):fim_formatAsUrl(a[0]).append(document.createTextNode(b))},3!==this.nodeType?$(this).text(""):null);3===this.nodeType&&$(this).replaceWith(a.html())}),jQuery.each({"~~":"del","*":"strong",_:"em"},function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+
a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(a){return $("<"+b+">").text(a[1])}).html())})}),jQuery.each(serverSettings.emoticons,function(a,b){c.find("strong, em, del").add(c.contents().add(c).filter(function(){return 3===this.nodeType})).each(function(){var a=fim_regexTokenizer(new RegExp(b.emoticonText.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,"\\$&"),"gi"),$(this).text(),function(){return $("<img>").attr("src",b.emoticonFile)},3!==this.nodeType?$(this).text(""):null);3===this.nodeType&&
$(this).replaceWith(a.html())})}),c.find("*").add(c).contents().filter(function(){return 3===this.nodeType}).each(function(){$(this).replaceWith(fim_regexTokenizer(new RegExp(/\n/g,"g"),$(this).text(),function(){return $("<br>")}))})}switch(b){case "table":var k=$('<tr style="word-wrap: break-word;">').attr({id:"archiveMessage' + messageId + '"}).append($("<td>").append(fim_buildUsernameTag($('<span class="userName userNameTable"></span>'),g,l,a))).append($('<td class="d-none d-sm-table-cell">').text(fim_dateFormat(d))).append($("<td>").append(fim_buildMessageLine(c,
e,g,f,d,l).append(c)));break;case "list":b=fim_buildMessageLine(c,e,g,f,d,l),$("iframe",b).length&&b.css("width","400px"),settings.showAvatars&&b.popover({content:function(){return fim_dateFormat($(this).attr("data-time"))},html:!1,trigger:"hover",placement:"bottom"}),d=settings.showAvatars?"":$('<span class="date">').css({"padding-right":"10px","letter-spacing":"-1px"}).text("@ ").append($("<em>").text(fim_dateFormat(d))),a=$('<span class="usernameDate">').append(fim_buildUsernameTag($("<span>"),
g,l,a,settings.showAvatars,!settings.showAvatars)),k=$("<span>").attr({id:"message"+e,"class":"messageLine"}),settings.showAvatars&&k.addClass("messageLineAvatar"),settings.showAvatars&&g==window.activeLogin.userData.id?k.addClass("messageLineReverse").append(b).append(a.append(d)):k.append(a.append(d)).append(b)}return k}function fim_buildUsernameTag(a,b,c,d,e,f){c||(c=fim_getUsernameDeferred(b));fim_buildUsernameTagPromise(a,b,c,d,e,f);return a}
function fim_buildUsernameTagPromise(a,b,c,d,e,f){var h=$.Deferred();void 0==e&&(e=!0);void 0==f&&(f=!0);d||(d="");$.when(c).then(function(c){var g=c[b].name+d,k=c[b].nameFormat,m=c[b].avatar?c[b].avatar:"images/blankperson.png";a.addClass("userName").addClass(e?" userNameAvatar":"").attr({style:f?k:"","data-style":k,"data-userId":b,"data-userName":g,"data-avatar":m,"data-bio":c[b].bio,"data-profile":c[b].profile,"data-joinDate":c[b].joinDate,tabindex:1E3}).append(e?$("<img>").attr({alt:g,src:m?m:
"images/blankperson.png"}):"").append(f?$("<span>").text(g):"");h.resolve(a)});return h.promise()}function fim_buildRoomNameTag(a,b,c){c||(c=fim_getRoomNameDeferred(b));fim_buildRoomNameTagPromise(a,b,c);return a}function fim_buildRoomNameTagPromise(a,b,c){var d=$.Deferred();$.when(c).then(function(c){c=c[String(b)].name;a.attr({"class":"roomName","data-roomId":b,"data-roomName":c,tabindex:1E3}).append($("<a>").attr("href","#room="+b).text(c));d.resolve(a)});return d.promise()}
function fim_buildMessageLine(a,b,c,d,e,f){var h=$("<span>");h.attr({"class":"messageText","data-messageId":b,"data-roomId":d,"data-time":e,tabindex:1E3}).append(a);if(window.userId==c&&window.activeLogin.userData.permissions.editOwnPosts)h.on("dblclick",function(){var a=$("<textarea>").on("keydown",function(g){13!=g.keyCode||g.shiftKey?27==g.keyCode&&($(this).replaceWith(fim_buildMessageLine(fim_messageFormat(a.text(),"list"),b,c,d,e,f)),g.preventDefault()):(fimApi.editMessage(d,b,{message:a.val()}),
$(this).replaceWith(fim_buildMessageLine(a.val(),b,c,d,e,f)),g.preventDefault())});fimApi.getMessages({roomId:d,id:b},{end:function(b){a.text(b[0].text)}});$.each(this.attributes,function(){a.attr(this.name,this.value)});a.css("width","100%");$(this).replaceWith(a)});$.when(f).then(function(a){!settings.disableFormatting&&settings.showAvatars&&a[c].messageFormatting&&h.attr("style",h.attr("style")+";"+a[c].messageFormatting).addClass("messageTextFormatted")});return h}
function fim_getUsernameDeferred(a){return $.when(Resolver.resolveUsersFromIds([a]))}function fim_getRoomNameDeferred(a){return $.when(Resolver.resolveRoomsFromIds([a]))}
...
image:/^(.+)\.(jpg|jpeg|gif|png|svg|svgz|bmp|ico)$/,youtubeFull:/^((http|https):(\/\/|)(www\.|)youtube\.com\/([^\ ]*?)(\?|\&)(w|v)=([a-zA-Z0-9-_]+)(&t=([a-zA-Z0-9]+))*)$/i,youtubeShort:/^((http|https):(\/\/|)(www\.|)youtu\.be\/([a-zA-Z0-9-_]+))$/i};window.isBlurred=!1;var favicon=$("#favicon").attr("href"),requestSettings={serverSentEvents:!1},timers={t1:!1},messageIndex={},settingsBitfield=$.getCookie("webpro_settings",50341888);
window.settings={theme:$.getCookie("webpro_theme","css"),audioVolume:$.getCookie("webpro_audioVolume",.5),disableFormatting:!!(settingsBitfield&16),disableImage:!!(settingsBitfield&32),disableVideos:!!(settingsBitfield&64),reversePostOrder:!!(settingsBitfield&1024),showAvatars:!!(settingsBitfield&2048),audioDing:!!(settingsBitfield&8192),disableRightClick:!!(settingsBitfield&1048576),webkitNotifications:!!(settingsBitfield&536870912)};
