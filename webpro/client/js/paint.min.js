...
jQuery.each({"```":"pre","`":"code"},function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(a){return $("<"+b+">").text(a[1])}).html())})}),c.find("*").add(c).contents().filter(function(){return 3===this.nodeType}).each(function(){var a=fim_regexTokenizer(regexs.url,$(this).text(),function(a){var b="";a[0].match(regexs.url2)&&(b=a[0].replace(regexs.url2,
...
this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(a){return $("<"+b+">").text(a[1])}).html())})}),jQuery.each(serverSettings.emoticons,function(a,b){c.find("strong, em, del").add(c.contents().add(c).filter(function(){return 3===this.nodeType})).each(function(){var a=fim_regexTokenizer(new RegExp(b.emoticonText.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,"\\$&"),"gi"),$(this).text(),function(){return $("<img>").attr("src",
b.emoticonFile)},3!==this.nodeType?$(this).text(""):null);3===this.nodeType&&$(this).replaceWith(a.html())})}),c.find("*").contents().filter(function(){return 3===this.nodeType}).each(function(){$(this).replaceWith(fim_regexTokenizer(new RegExp(/\n/g,"g"),$(this).text(),function(){return $("<br>")}))})}switch(b){case "table":var m=$('<tr style="word-wrap: break-word;">').attr({id:"archiveMessage' + messageId + '"}).append($("<td>").append(fim_buildUsernameTag($('<span class="userName userNameTable"></span>'),
