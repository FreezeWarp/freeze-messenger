...
function fim_messageFormat(a,b){var c=a.text,d=a.time,e=a.id,f=a.roomId,h=a.flag,g=Number(a.userId);a=Number(a.anonId);var k=fim_getUsernameDeferred(g);c=c.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br />");switch(h){case "source":c=fim_youtubeParse(c);break;case "image":c=fim_formatAsImage(c);break;case "video":c=fim_formatAsVideo(c);break;case "audio":c=fim_formatAsAudio(c);break;case "email":c=fim_formatAsEmail(c);break;case "url":case "text":case "html":case "archive":case "application":case "other":c=
fim_formatAsUrl(c);break;default:c=$("<span>").text(c),c.html(c.text().replace(regexs.url,function(a){if(a.match(regexs.url2)){var b=a.replace(regexs.url2,"$2");a=a.replace(regexs.url2,"$1")}else b="";return a.match(regexs.youtubeFull)||a.match(regexs.youtubeShort)?fim_youtubeParse(a).prop("outerHTML")+b:a.match(regexs.image)?fim_formatAsImage(a).prop("outerHTML")+b:fim_formatAsUrl(a).prop("outerHTML")+b})),/^\/me/.test(c.text())?(c.text(c.text().replace(/^\/me/,"")),$.when(k).then(function(a){c.html($('<span style="color: red; padding: 10px; font-weight: bold;">').text("* "+
a[g].name+" "+c.text()).prop("outerHTML"))})):/^\/topic/.test(c.text())&&(c.text(c.text().replace(/^\/topic/,"")),$.when(k).then(function(a){c.html($('<span style="color: red; padding: 10px; font-weight: bold;">').text("* "+a[g].name+' changed the topic to "'+c.text().trim()+'".').prop("outerHTML"))})),jQuery.each(serverSettings.emoticons,function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){$(this).replaceWith($(this).text().replace(new RegExp(b.emoticonText.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,
"\\$&"),"gi"),function(){return $("<img>").attr("src",b.emoticonFile).prop("outerHTML")}))})})}switch(b){case "table":var l=$('<tr style="word-wrap: break-word;">').attr({id:"archiveMessage' + messageId + '"}).append($("<td>").append(fim_buildUsernameTag($('<span class="userName userNameTable"></span>'),g,k,a))).append($('<td class="d-none d-sm-table-cell">').text(fim_dateFormat(d))).append($("<td>").append(fim_buildMessageLine(c,e,g,f,d,k).append(c)));break;case "list":b=fim_buildMessageLine(c,e,
g,f,d,k),settings.showAvatars&&b.popover({content:function(){return fim_dateFormat($(this).attr("data-time"))},html:!1,trigger:"hover",placement:"bottom"}),l=$("<span>").attr({id:"message"+e,"class":"messageLine"+(settings.showAvatars?" messageLineAvatar":"")}).append($('<span class="usernameDate">').append(fim_buildUsernameTag($("<span>"),g,k,a,settings.showAvatars,!settings.showAvatars)).append(settings.showAvatars?"":$('<span class="date">').css({"padding-right":"10px","letter-spacing":"-1px"}).text("@ ").append($("<em>").text(fim_dateFormat(d))))).append(b)}return l}
function fim_buildUsernameTag(a,b,c,d,e,f){c||(c=fim_getUsernameDeferred(b));fim_buildUsernameTagPromise(a,b,c,d,e,f);return a}
function fim_buildUsernameTagPromise(a,b,c,d,e,f){var h=$.Deferred();void 0==e&&(e=!0);void 0==f&&(f=!0);console.log("aid",d);d||(d="");$.when(c).then(function(c){var g=c[b].name+d,l=c[b].nameFormat,m=c[b].avatar?c[b].avatar:"images/blankperson.png";a.attr({"class":"userName"+(e?" userNameAvatar":""),style:f?l:"","data-style":l,"data-userId":b,"data-userName":g,"data-avatar":m,"data-bio":c[b].bio,"data-profile":c[b].profile,"data-joinDate":c[b].joinDate,tabindex:1E3}).append(e?$("<img>").attr({alt:g,
src:m?m:"images/blankperson.png"}):"").append(f?$("<span>").text(g):"");h.resolve(a)});return h.promise()}function fim_buildRoomNameTag(a,b,c){c||(c=fim_getRoomNameDeferred(b));fim_buildRoomNameTagPromise(a,b,c);return a}function fim_buildRoomNameTagPromise(a,b,c){var d=$.Deferred();$.when(c).then(function(c){c=c[String(b)].name;a.attr({"class":"roomName","data-roomId":b,"data-roomName":c,tabindex:1E3}).append($("<a>").attr("href","#room="+b).text(c));d.resolve(a)});return d.promise()}
