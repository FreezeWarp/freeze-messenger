...
function fim_regexTokenizer(a,b,c,d){d||(d=$("<span>"));for(prevIndex=0;null!==(result=a.exec(b));)d.append(document.createTextNode(b.substring(prevIndex,result.index))),d.append(c(result)),prevIndex=a.lastIndex;d.append(document.createTextNode(b.substring(prevIndex)));return d}
function fim_messageFormat(a,b){var c=a.text,d=a.time,e=a.id,f=a.roomId,h=a.flag,g=Number(a.userId);a=Number(a.anonId);var l=fim_getUsernameDeferred(g);switch(h){case "source":c=fim_youtubeParse(c);break;case "image":c=fim_formatAsImage(c);break;case "video":c=fim_formatAsVideo(c);break;case "audio":c=fim_formatAsAudio(c);break;case "email":c=fim_formatAsEmail(c);break;case "url":case "text":case "html":case "archive":case "application":case "other":c=fim_formatAsUrl(c);break;default:c=$("<span>").text(c),
jQuery.each({"`":"code"},function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(a){return $("<"+b+">").text(a[1])}).html())})}),c.find('*:not(:has("*"))').add(c.contents().add(c).filter(function(){return 3===this.nodeType})).each(function(){var a=fim_regexTokenizer(regexs.url,$(this).text(),function(a){var b="";a[0].match(regexs.url2)&&(b=a[0].replace(regexs.url2,
"$2"),a[0]=a[0].replace(regexs.url2,"$1"));return a[0].match(regexs.youtubeFull)||a[0].match(regexs.youtubeShort)?fim_youtubeParse(a[0]).append(document.createTextNode(b)):a[0].match(regexs.image)?fim_formatAsImage(a[0]).append(document.createTextNode(b)):fim_formatAsUrl(a[0]).append(document.createTextNode(b))},3!==this.nodeType?$(this).text(""):null);3===this.nodeType&&$(this).replaceWith(a.html())}),jQuery.each({"*":"strong",_:"em"},function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp("\\"+
a+"([\\s\\S]+?)\\"+a,"g"),$(this).text(),function(c){console.log("match",a,b,c);return $("<"+b+">").text(c[1])}).html())})}),jQuery.each(serverSettings.emoticons,function(a,b){c.contents().filter(function(){return 3===this.nodeType}).each(function(){return $(this).replaceWith(fim_regexTokenizer(new RegExp(b.emoticonText.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,"\\$&"),"gi"),$(this).text(),function(){return $("<img>").attr("src",b.emoticonFile)}).html())})}),c.find("*").contents().filter(function(){return 3===
this.nodeType}).each(function(){$(this).replaceWith(fim_regexTokenizer(new RegExp(/\n/g,"g"),$(this).text(),function(){return $("<br>")}))}),c.find('*:not(:has("*"))').each(function(){fim_regexTokenizer(new RegExp(/\n/g,"g"),$(this).text(),function(){return $("<br>")},$(this).text(""))})}switch(b){case "table":var k=$('<tr style="word-wrap: break-word;">').attr({id:"archiveMessage' + messageId + '"}).append($("<td>").append(fim_buildUsernameTag($('<span class="userName userNameTable"></span>'),g,
l,a))).append($('<td class="d-none d-sm-table-cell">').text(fim_dateFormat(d))).append($("<td>").append(fim_buildMessageLine(c,e,g,f,d,l).append(c)));break;case "list":b=fim_buildMessageLine(c,e,g,f,d,l),settings.showAvatars&&b.popover({content:function(){return fim_dateFormat($(this).attr("data-time"))},html:!1,trigger:"hover",placement:"bottom"}),k=$("<span>").attr({id:"message"+e,"class":"messageLine"+(settings.showAvatars?" messageLineAvatar":"")}).append($('<span class="usernameDate">').append(fim_buildUsernameTag($("<span>"),
g,l,a,settings.showAvatars,!settings.showAvatars)).append(settings.showAvatars?"":$('<span class="date">').css({"padding-right":"10px","letter-spacing":"-1px"}).text("@ ").append($("<em>").text(fim_dateFormat(d))))).append(b)}return k}function fim_buildUsernameTag(a,b,c,d,e,f){c||(c=fim_getUsernameDeferred(b));fim_buildUsernameTagPromise(a,b,c,d,e,f);return a}
function fim_buildUsernameTagPromise(a,b,c,d,e,f){var h=$.Deferred();void 0==e&&(e=!0);void 0==f&&(f=!0);d||(d="");$.when(c).then(function(c){var g=c[b].name+d,k=c[b].nameFormat,m=c[b].avatar?c[b].avatar:"images/blankperson.png";a.attr({"class":"userName"+(e?" userNameAvatar":""),style:f?k:"","data-style":k,"data-userId":b,"data-userName":g,"data-avatar":m,"data-bio":c[b].bio,"data-profile":c[b].profile,"data-joinDate":c[b].joinDate,tabindex:1E3}).append(e?$("<img>").attr({alt:g,src:m?m:"images/blankperson.png"}):
"").append(f?$("<span>").text(g):"");h.resolve(a)});return h.promise()}function fim_buildRoomNameTag(a,b,c){c||(c=fim_getRoomNameDeferred(b));fim_buildRoomNameTagPromise(a,b,c);return a}function fim_buildRoomNameTagPromise(a,b,c){var d=$.Deferred();$.when(c).then(function(c){c=c[String(b)].name;a.attr({"class":"roomName","data-roomId":b,"data-roomName":c,tabindex:1E3}).append($("<a>").attr("href","#room="+b).text(c));d.resolve(a)});return d.promise()}
