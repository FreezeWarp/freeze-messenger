...
new standard;window.popup=new popup;requestSettings.serverSentEvents="undefined"==typeof window.EventSource?!1:window.serverSettings.requestMethods.serverSentEvents;window.serverSettings.installUrl!=window.location.protocol+"//"+window.location.host+window.directory&&dia.error(window.phrases.errorBadInstall);$(document).ready(function(){fim_renderHandlebarsInPlace($("#entry-template"));var a=function(a,b){dia.full({title:"Link to this Message",content:$("<span>").text("This message can be bookmarked using the following archive link:").append($("<br>"),
...
$("#privateRoomForm input[name=userName]").autocompleteHelper("users");$("#privateRoomForm").submit(function(){console.log("form submitted");var a=$("#privateRoomForm input[name=userName]").val(),b=$("#privateRoomForm input[name=userName]").attr("data-id");whenUserIdAvailable=function(a){window.location.hash="#room=p"+[window.userId,a].join()};!b&&a?whenUserIdAvailable(b):a?$.when(Resolver.resolveUsersFromNames([a]).then(function(b){whenUserIdAvailable(b[a].id)})):dia.error("Please enter a username.");
return!1});window.webproDisplay.fontSize&&$("body").css("font-size",window.webproDisplay.fontSize+"em");settings.disableFx&&(jQuery.fx.off=!0);$(window).bind("resize",windowResize);$(window).bind("hashchange",fim_hashParse);document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState?windowBlur():windowFocus()});showLogin=function(){window.location.hash.match(/\#sessionHash=/)?standard.initialLogin({grantType:"access_token",sessionHash:window.location.hash.match(/\#sessionHash=([^\#]+)/)[1],
error:function(){window.userId||popup.login()}}):$.cookie("webpro_refreshToken")?standard.initialLogin({grantType:"refresh_token",refreshToken:$.cookie("webpro_refreshToken"),error:function(){window.userId||popup.login()}}):$.cookie("webpro_username")?standard.initialLogin({username:$.cookie("webpro_username"),password:$.cookie("webpro_password"),error:function(){window.userId||popup.login()}}):popup.login()};showLogin();return!1})},function(){$q("Loading failed. Please refresh.")});
