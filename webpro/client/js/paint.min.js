...
function fim_renderHandlebarsInPlace(a,b){b=a.attr("id");var c=a.html();c=Handlebars.compile(c);$("#active-"+b).remove();$('<div id="active-'+b+'">'+c(Object.assign({},window.phrases,{serverSettings:window.serverSettings,activeLogin:window.activeLogin}))+"</div>").insertAfter(a)}
function fim_openView(a,b){if($(".fim-activeView").attr("id")=="active-view-"+a)jQuery.each(b,function(b,d){b="set"+b.charAt(0).toUpperCase()+b.slice(1);if("undefined"!=typeof popup[a]&&"undefined"!=typeof popup[a][b])popup[a][b](d)});else if(tag=$("#view-"+a),0<tag.length)fim_closeView(),fim_renderHandlebarsInPlace(tag),$("#active-view-"+a).addClass("fim-activeView"),"undefined"!=typeof popup[a]&&popup[a].init(b);else throw"Unknown view.";}
function fim_closeView(){$(".fim-activeView").each(function(){var a=$(this).attr("id").slice(12);"undefined"!=typeof popup[a]&&"undefined"!=typeof popup[a].close&&popup[a].close(options);$(this).remove()})}
...
$.when($.ajax({url:"client/data/config.json",dataType:"json",success:function(a){window.fim_config=a}}),$.ajax({url:"client/data/language_enGB.json",dataType:"json",success:function(a){window.phrases=a}}),$.ajax({url:"client/js/fim-dev/fim-api.js",dataType:"script"}),$.ajax({url:"client/js/fim-dev/fim-apiHelper.js",dataType:"script"}),$.ajax({url:"client/js/fim-dev/fim-standard.js",dataType:"script"}),$.ajax({url:"client/js/fim-dev/fim-popup.js",dataType:"script"}),$.ajax({url:"client/js/fim-dev/fim-popup-room.js",
dataType:"script"}),$.ajax({url:"client/js/fim-dev/fim-loader.js",dataType:"script"}),$.ajax({url:window.directory+"api/serverStatus.php",dataType:"json",success:function(a){window.serverSettings=a.serverStatus}})).then(function(){window.fimApi=new fimApi;fimApi.registerDefaultExceptionHandler(function(a){dia.exception(a)});window.standard=new standard;window.popup=new popup;window.serverSettings.installUrl!=window.location.protocol+"//"+window.location.host+window.directory&&dia.error(window.phrases.errorBadInstall);
$(document).ready(function(){fim_renderHandlebarsInPlace($("#entry-template"));var a=function(a,b){dia.full({title:"Link to this Message",content:$("<span>").text("This message can be bookmarked using the following archive link:").append($("<br>"),$("<br>"),$("<input>").attr({type:"text",value:currentLocation+"#page=archive#room="+a+"#message="+b,autofocus:!0,id:"messageLink-"+a+"-"+b,style:"width: 100%;"})).prop("outerHTML"),oF:function(){$("#messageLink-"+a+"-"+b).focus().select()}})},b=function(a,
b){dia.confirm({text:"Are you sure you want to delete this message?","true":function(){standard.deleteMessage(a,b)}})},c=function(a){$("#message"+a+" .messageText").dblclick()},d={disabled:"bg-inverse",visible:"bg-primary",notSelectable:"not-selectable"};$.contextMenu({classNames:d,selector:".messageText",items:{delete:{name:"Delete",callback:function(){b($(this).attr("data-roomid"),$(this).attr("data-messageid"))}},link:{name:"Link",callback:function(){a($(this).attr("data-roomid"),$(this).attr("data-messageid"))}},
edit:{name:"Edit",callback:function(){c($(this).attr("data-messageid"))},visible:function(){return $(this).parent(".messageLine").find(".userName").attr("data-userid")==window.userId&&window.activeLogin.userData.permissions.editOwnPosts}}}});$.contextMenu({classNames:d,selector:".messageText img",items:{delete:{name:"Delete",callback:function(){b($(this).closest(".messageText").attr("data-roomid"),$(this).closest(".messageText").attr("data-messageid"))}},link:{name:"Link",callback:function(){a($(this).closest(".messageText").attr("data-roomid"),
...
$("<br>"),$("<br>"),$("<input>").attr({type:"text",name:"url",value:$(this).attr("src"),style:"width: 100%"})).prop("outerHTML"),width:800,position:"top",oF:function(){$("input[name=url]",this).first().focus()}})}}}});$.contextMenu({classNames:d,selector:".messageText a",items:{delete:{name:"Delete",callback:function(){b($(this).closest(".messageText").attr("data-roomid"),$(this).closest(".messageText").attr("data-messageid"))}},link:{name:"Link",callback:function(){console.log($(this),$(this).closest(".messageText"),
$(this).closest(".messageText").attr("data-roomid"));a($(this).closest(".messageText").attr("data-roomid"),$(this).closest(".messageText").attr("data-messageid"))}},edit:{name:"Edit",callback:function(){c($(this).closest(".messageText").attr("data-messageid"))},visible:function(){return $(this).closest(".messageLine").find(".userName").attr("data-userid")==window.userId&&window.activeLogin.userData.permissions.editOwnPosts}},click:{name:"URL",callback:function(){dia.full({title:"Copy URL",position:"top",
content:$("<input>").attr({type:"text",name:"url",value:$(this).attr("href"),style:"width: 100%;"}).prop("outerHTML"),width:800,oF:function(){$("input[name=url]",this).first().focus()}})}}}});$.contextMenu({classNames:d,selector:".userName",items:{profile:{name:"Profile",callback:function(){$.when(Resolver.resolveUsersFromIds([userId])).then(function(a){dia.full({title:"User Profile",id:"messageLink",content:a[userId].profile?'<iframe src="'+a[userId].profile+'" style="width: 100%; height: 90%;" /><br /><a href="'+
...
callback:function(){standard.banUser($("data-userid"))},visible:function(){return!1}},ignore:{name:"Ignore",callback:function(){dia.alert("This functionality is not yet implemented.")}}}});$("#privateRoomForm input[name=userName]").autocompleteHelper("users");$("#privateRoomForm").submit(function(){console.log("form submitted");var a=$("#privateRoomForm input[name=userName]").val(),b=$("#privateRoomForm input[name=userName]").attr("data-id");whenUserIdAvailable=function(a){window.location.hash="#room=p"+
[window.userId,a].join()};!b&&a?whenUserIdAvailable(b):a?$.when(Resolver.resolveUsersFromNames([a]).then(function(b){whenUserIdAvailable(b[a].id)})):dia.error("Please enter a username.");return!1});window.webproDisplay.fontSize&&$("body").css("font-size",window.webproDisplay.fontSize+"em");settings.disableFx&&(jQuery.fx.off=!0);$(window).bind("resize",windowResize);$(window).bind("hashchange",fim_hashParse);document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState?
windowBlur():windowFocus()});$("#showMoreRooms").bind("click",function(){$("#roomListShort").slideUp();$("#roomListLong").slideDown()});$("#showFewerRooms").bind("click",function(){$("#roomListLong").slideUp();$("#roomListShort").slideDown()});showLogin=function(){window.location.hash.match(/\#sessionHash=/)?standard.initialLogin({grantType:"access_token",sessionHash:window.location.hash.match(/\#sessionHash=([^\#]+)/)[1],error:function(){window.userId||popup.login()}}):$.cookie("webpro_refreshToken")?
