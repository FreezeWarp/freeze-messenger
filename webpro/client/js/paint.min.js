...
position:"top",content:$("<input>").attr({type:"text",name:"url",value:$(this).attr("href"),style:"width: 100%;"}).prop("outerHTML"),width:800,oF:function(){$("input[name=url]",this).first().focus()}})},visible:function(a,b){console.log("visible",$(this),b);return"IMG"===$(this).prop("tagName")||"A"===$(this).prop("tagName")}}}});$.contextMenu({classNames:a,selector:".userName",events:b,items:{profile:{name:"Profile",callback:function(){$.when(Resolver.resolveUsersFromIds([userId])).then(function(a){dia.full({title:"User Profile",
...
{end:function(){dia.info("You have added "+a.prop("outerHTML")+" to your ignore list. They will not be able to send you private messages.")}})},visible:function(){return $(this).attr("data-userId")!=window.activeLogin.userData.id&&0>window.activeLogin.userData.ignoredUsers.indexOf(Number($(this).attr("data-userId")))}},unignore:{name:"Unignore",callback:function(){var a=$(this);fimApi.editUserOptions("delete",{ignoreList:[$(this).attr("data-userId")]},{end:function(){dia.info("You have removed "+
a.prop("outerHTML")+" from your ignore list.")}})},visible:function(){return $(this).attr("data-userId")!=window.activeLogin.userData.id&&0<=window.activeLogin.userData.ignoredUsers.indexOf(Number($(this).attr("data-userId")))}}}});window.webproDisplay.fontSize&&$("body").css("font-size",window.webproDisplay.fontSize+"em");settings.disableFx&&(jQuery.fx.off=!0);$(window).bind("hashchange",fim_hashParse);showLogin=function(){window.location.hash.match(/\#sessionHash=/)?standard.initialLogin({grantType:"access_token",
