...
b,c,d,e,f,g)),h.preventDefault()):(fimApi.editMessage(e,c,{message:a.val()}),$(this).replaceWith(fim_buildMessageLine(a.val(),b,c,d,e,f,g)),h.preventDefault())});fimApi.getMessages({roomId:e,id:c},{end:function(b){a.text(b[0].text)}});$.each(this.attributes,function(){a.attr(this.name,this.value)});a.css("width","100%");$(this).replaceWith(a)});$.when(g).then(function(a){!settings.disableFormatting&&settings.showAvatars&&a[d].messageFormatting&&k.attr("style",k.attr("style")+";"+a[d].messageFormatting).addClass("messageTextFormatted")});
return k}function fim_getUsernameDeferred(a){return $.when(Resolver.resolveUsersFromIds([a]))}function fim_getRoomNameDeferred(a){return $.when(Resolver.resolveRoomsFromIds([a]))}
...
var favicon=$("#favicon").attr("href"),requestSettings={serverSentEvents:!1},timers={t1:!1},messageIndex={},settingNames={disableFormatting:16,disableImages:32,disableVideos:64,alternateSelfPosts:512,reversePostOrder:1024,showAvatars:2048,hideTimes:4096,audioDing:8192,disableRightClick:1048576,webkitNotifications:536870912},settingsBitfield=$.cookie("webpro_settings");
settingsBitfield||0===settingsBitfield||(settingsBitfield=settingNames.showAvatars+settingNames.audioDing+settingNames.alternateSelfPosts,$.cookie("webpro_settings",settingsBitfield));
window.settings={theme:$.getCookie("webpro_theme","css"),audioVolume:$.getCookie("webpro_audioVolume",.5),alternateSelfPosts:!!(settingsBitfield&settingNames.alternateSelfPosts),disableFormatting:!!(settingsBitfield&settingNames.disableFormatting),disableImages:!!(settingsBitfield&settingNames.disableImages),disableVideos:!!(settingsBitfield&settingNames.disableVideos),reversePostOrder:!!(settingsBitfield&settingNames.reversePostOrder),showAvatars:!!(settingsBitfield&settingNames.showAvatars),hideTimes:!!(settingsBitfield&
settingNames.hideTimes),audioDing:!!(settingsBitfield&settingNames.audioDing),disableRightClick:!!(settingsBitfield&settingNames.disableRightClick),webkitNotifications:!!(settingsBitfield&settingNames.webkitNotifications)};if(1<window.settings.audioVolume||0>window.settings.audioVolume)console.error("audioVolume was "+window.settings.audioVolume+"; set to .5"),window.settings.audioVolume=.5;
