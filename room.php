...
/* Scratch:
 * alright, so basic idea here is (method URI body = URI body):
 * PUT /room/ name=fun = room.php?action=create name=fun
 * POST /room/1/ name=fun = room.php?action=edit&roomIds=1 name=fun
 * DELETE /room/1/ = room.php?action=delete&roomIds=1
 * GET /room/1/ = room.php?action=get&roomIds=1
 *
 * Different request heads:
 * /room/roomIds[]=1&roomIds[]=2&roomIds[]=3
 * /room/roomNames=hello
 * /room/roomNames[]=hello&roomNames[]=goodbye
 * /room/permFilter=post
 * /room/
 *
...
$requestG = fim_sanitizeGPC('g', array(
  /* Action */
  'action' => array(
    // 'c reate', 'edit', 'delete', and 'undelete' MUST be part of a POST request. 'get' MUST be part of a GET request. However, this parameter, as with other get parameters, are URL parameters -- and thus read using $_GET.
    'valid' => array(
      'get',
      'create', 'edit',
      'delete', 'undelete'
