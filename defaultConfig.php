...
$defaultConfig = array(
    ...
    'censorBlackWhiteListsCacheRefresh' => 1800, // int The number of seconds after which the censor black-white lists cache (that is, which rooms have opted-in to black lists and opted-out of white lists) will be refreshed. Because this is a *full* cache, the table will be read in its entirety every time this amount of time elapses, so it should only be so low. It shouldn't be too high, however, because rooms will experience, at most, this delay between when they update their used lists and when those updated lists go into effect.
